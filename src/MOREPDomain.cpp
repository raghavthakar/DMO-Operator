...
void MOREPDomain::loadConfig(const std::string& filename) {
    ...
    yLength = dimensions["yLength"].as<double>(); // Read the MOREPDomain dimensions into the object
    ...
    bool randomPOIConfig = config["MOREPDomain"]["randomPOIConfig"].as<bool>();

    // Load up an MOREPDomain configuration
    ...
    if (randomPOIConfig == true) {
        ...
        for (int classID = 0; classID < numberOfClassIDs; ++classID) {
            ...
            for (int i = 0; i < poisToAdd; ++i) {
                ...
                if (config["MOREPDomain"]["eternalPOIs"].as<bool>() == true) {
                    // set observable window to eternity
                    observableWindow = std::make_pair(0, std::numeric_limits<int>::max());
                }
                else {
                    observableWindow = std::make_pair(config["MOREPDomain"]["observableWindow"][0].as<int>(), 
                                                    config["MOREPDomain"]["observableWindow"][1].as<int>());
                }

                // Put into list
                pois.emplace_back(poi_num++, classID, poi_x, poi_y, 
                    config["MOREPDomain"]["observationRadius"].as<double>(),
                    config["MOREPDomain"]["coupling"].as<int>(),
                    config["MOREPDomain"]["reward"].as<int>(),
                    observableWindow,
                    config["MOREPDomain"]["exactCouplingNeeded"].as<bool>(),
                    config["MOREPDomain"]["rewardOnce"].as<bool>()); // Create POI object and add to vector
            }
        }
    }
    // Else, read the configs from poi list in config and add pois
    else {
        for (const auto& poi: config["MOREPDomain"]["POIs"]) {
...
std::vector<int> MOREPDomain::getRewards(std::vector<std::pair<double, double>> agentPositions,
                                        int stepNumber) {
...
void MOREPDomain::printInfo() const {
    std::cout << "POIs in the MOREPDomain:" << std::endl;
    for (const auto& poi : pois) {
...
std::vector<POI> MOREPDomain::getPOIs() {
    return pois;
}

std::pair<int, int> MOREPDomain::getDimensions() {
